  ...
  const validate = () => {
    ...
    if (!userInput.email) {
      ...
      (!userInput.email.endsWith('@gmail.com') &&
        !userInput.email.endsWith('@yahoo.com') &&
        !userInput.email.endsWith('@hotmail.com') &&
        !userInput.email.endsWith('@icloud.com'))
    ) {
      newErrors.email = 'Please enter a valid email address.';
    }
    if (!userInput.password) {
  ...
  const handleSubmit = async (event) => {
    ...
    if (validate()) {
      try {
        ...
        setUserInput({ username: '', email: '', password: '', terms: false });
        console.log("Success: Registered successfully!"); // Debug log
        toastNotifications('success', 'Registered successfully! Please login to continue.');
      } catch (error) {
        if (error.response) {
          console.log('Error:', error.response.data);
          if (error.response.data.username) {
            console.log("Username error: ", error.response.data.username[0]); // Debug log
            toastNotifications('error', error.response.data.username[0]);
          } else if (error.response.data.email) {
            console.log("Email error: ", error.response.data.email[0]); // Debug log
            toastNotifications('error', error.response.data.email[0]);
          }
        } else {
          console.log('Error:', error.message);
  ...
  return (
    <div className="Signup-container">
      <div>
        <div className="Signup-Quote">
          <span className="LeftSpan">Create Account</span><br />
          <span className="SmallQuote">Your python journey starts here</span><br />
        </div>
        <form onSubmit={handleSubmit}>
          <div>
            <input
