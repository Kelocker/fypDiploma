...
const Compiler = () => {
    ...
    //     if (!snippetId) {
    //         console.error('Snippet ID is undefined or null');
    //         setError('Snippet ID not found after submission.');
    //         setLoading(false);
    //         return;
    //     }
    //     try {
    //         const resultResponse = await axios.get(`http://127.0.0.1:8000/api/result/`);
    //         setResults(resultResponse.data.results);
    //     } catch (error) {
    //         console.error('Failed to fetch results:', error);
    //         setError(error.response?.data?.detail || 'Failed to load results. Please try again.');
    //     } finally {
    //         setLoading(false);
    //     }
    // };
